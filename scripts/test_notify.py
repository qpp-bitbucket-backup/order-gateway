...
def test_direct(task_name: str, webhook_log_id: int, order_id: str, event_status: str):
    """Directly call the task function (no Celery broker)."""
    from app.tasks.notifications import notify_oms, notify_vfs

    task = notify_oms if task_name == "oms" else notify_vfs
    result = task(webhook_log_id=webhook_log_id, order_id=order_id, event_status=event_status)
    logger.info(f"Result: {result}")


def test_celery_delay(task_name: str, webhook_log_id: int, order_id: str, event_status: str):
    """Send via Celery delay() (requires RabbitMQ + a worker consuming order_notifying)."""
    from app.tasks.notifications import notify_oms, notify_vfs

    task = notify_oms if task_name == "oms" else notify_vfs
    async_result = task.delay(webhook_log_id=webhook_log_id, order_id=order_id, event_status=event_status)
...
if __name__ == "__main__":
    ...
    if args.delay:
        test_celery_delay(args.task, args.webhook_log_id, args.order_id, args.status)
    else:
        test_direct(args.task, args.webhook_log_id, args.order_id, args.status)
