"""
Test script for validate_order Celery task.

Usage:
    python test_validate_order.py                          # 用預設 order_id 測試
    python test_validate_order.py --order-id YOUR_ORDER_ID # 指定 order_id
    python test_validate_order.py --direct                 # 直接調用函數（不走 Celery）
    python test_validate_order.py --delay                  # 發送到 broker（需要 RabbitMQ + worker）
"""
import os
import sys
...
def get_order_data(order_id: str) -> dict:
    """Build the order_data payload for validate_order."""
...
def test_direct(order_id: str):
    """直接調用 validate_order 函數（不走 Celery broker）。"""
    from app.tasks.orders import validate_order
...
def test_celery_apply(order_id: str):
    ...
    result = validate_order.apply(args=[order_data])
...
def test_celery_delay(order_id: str):
    ...
    async_result = validate_order.delay(order_data)
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Test validate_order task")
    parser.add_argument(
        "--order-id",
        type=str,
    ...
    logger.info(f"=== Testing validate_order for order_id={args.order_id} ===")
