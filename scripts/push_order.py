...
def get_order_data(order_id: str) -> dict:
    """Build the order_data payload for push_order."""
...
def test_direct(order_id: str):
    """直接調用 push_order 函數（不走 Celery broker）。"""
...
def test_celery_apply(order_id: str):
    ...
    result = push_order.apply(args=[order_data])
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Test push_order task")
    parser.add_argument(
        "--order-id",
        type=str,
    ...
    logger.info(f"=== Testing push_order for order_id={args.order_id} ===")
