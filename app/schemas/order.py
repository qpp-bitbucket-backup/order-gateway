...
class PlatformOrderSummary(BaseModel):
    ...
    parentStoreOrderId: Optional[str] = Field(None, description="Base card order's store order ID (parallel card orders only)")
...
class OrderUpdateResponse(BaseModel):
    ...
    cancelledOrderId: Optional[str] = Field(
        None,
        description="Order ID of the cancelled original order, present only for content updates (cancel + recreate)",
    )
